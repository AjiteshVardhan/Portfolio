...
import {
  ...
  FileText, Image as ImageIcon, Info, Box as BoxIcon
} from "lucide-react";
...
const PROJECTS_RAW = [
  /* ─── PROJECT 1 ────────────────────────────────────────────────── */
  {
    ...
    models3d: [
       { url: "/models/Flow straightener.stl", caption: "Wind Tunnel Flow Straightener" },
    ],
  },
  ...
  /* ─── PROJECT 2 ────────────────────────────────────────────────── */
  {
    ...
    models3d: [
      // { url: "/models/venturi.stl", caption: "Venturi CAD Assembly" },
    ],
  },
  ...
  /* ─── PROJECT 3 ────────────────────────────────────────────────── */
  {
    ...
    codeSnippet: null,
    models3d: [
      // { url: "/models/bridge.stl", caption: "Bridge CAD" },
    ],
  },
  ...
  /* ─── PROJECT 4 ────────────────────────────────────────────────── */
  {
    ...
    codeSnippet: null,
    models3d: [
      // { url: "/models/bracket.stl", caption: "Acrylic Bracket CAD" },
    ],
  },
];
...
function ProjectModal({ project, onClose }) {
  ...
  const hasMedia = images.length > 0 || docs.length > 0 || codeSnippet || models3d.length > 0;
        ...
        <div className="px-7 py-6 flex flex-col gap-7">
          ...
          {models3d.length > 0 && (
            <div>
              <p style={{ ...MONO, fontSize:10 }} className="text-slate-500 tracking-widest uppercase mb-3">
                3D Model
                <span className="text-slate-700 ml-2">({models3d.length})</span>
              </p>
              <div className="flex flex-col gap-5">
                {models3d.map((model, i) => (
                  <div key={i}>
                    <CADViewer url={model.url} accentHex={accent.hex} height="360px" />
                    {model.caption && (
                      <p style={{ ...MONO, fontSize:10 }} className="text-slate-500 mt-2 text-center">{model.caption}</p>
                    )}
                  </div>
                ))}
              </div>
          ...
          {/* 5. FALLBACK — shown only when all media fields are empty */}
          {!hasMedia && (
...
function ProjectCard({ project, onClick, orderLabel }) {
  ...
  const modelCount  = models3d.length;
      ...
      <div className="flex flex-col flex-1 p-4 sm:p-5 md:p-6">
        ...
        <div className="flex items-center justify-between gap-2 pt-3 border-t border-slate-700">
          ...
          <div style={MONO} className="text-xs text-slate-600 shrink-0 flex items-center gap-2">
            ...
            {modelCount > 0 && <span className="flex items-center gap-1" style={{ color:accent.hex }}><BoxIcon size={10} />{modelCount}</span>}
            {imgCount === 0 && docCount === 0 && !hasCode && modelCount === 0 && <span className="text-slate-700">No media</span>}
          </div>
        </div>
      </div>
    </div>
